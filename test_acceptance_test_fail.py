...
@allure.feature("=== Тест fault ===")
def test_acceptance_test(capsys):
    hospital = Hospital()
    expected_outputs = read_expected_output('expected_output_test_fail.txt')

    def run_and_assert(command, *args, expected_output_index):
        print(f"\n {command}")
        captured_before = capsys.readouterr()
        #hospital.execute_command(command, patient_ID=patient_id)
    ...
    # Шаг 6:
    # в ожидаемом ответе допущена ошибка
    #actual out result Сеанс завершён. - Это правильный вывод результата
    #expected out result Сеанс завершён1. - Это неверный результат
